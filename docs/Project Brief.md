...
* **Ethical Safety by Design:** To treat privacy not as a feature, but as a hard-coded requirement through on-watch STT processing, strict data minimization, and transparent educator-led risk mitigation.
...
* **Zero-Config Cloud Software:** The backend is hosted as a secure Cloud Backend, eliminating the setup and maintenance complexity of local servers. AI orchestration (including LLMs) runs in the cloud with deterministic script guardrails, while speech-to-text is handled on-device on the smartwatch for maximum privacy.
...
* **Data Sovereignty & Minimization:** To neutralize surveillance risks, speech-to-text transcription occurs directly on the smartwatch using an embedded local model (Moonshine). Only resulting text transcripts are sent to the Cloud Backend. All cloud processing is transient, deleting transcripts immediately from memory after processing. No raw audio ever leaves the watch, and no data is stored permanently on our servers unless explicitly opted into by the caregiver.
* **Audio Privacy:** Raw audio is processed strictly on-device on the smartwatch and is deleted immediately after transcription. No raw audio is ever transmitted to the cloud.
